...
const Axis = ({ data, position, title }) => {
  ...
  return (
    <g className="axis" transform={`translate(${position}, 0)`}>
      ...
      <text className="axis-title" x="0" y="-4">
        {title}
      </text>
    </g>
  );
...
const DiscGraphPlus = ({ data, title }) => {
  ...
  const getCoordinate = (value, axis) => {
    ...
    const values = Object.keys(axisCoordinates[axis])
      .map(Number)
      .sort((a, b) => a - b);
    ...
    if (higherValue === Infinity)
      return axisCoordinates[axis][values[values.length - 1]];
    ...
    return (
      lowerY +
      ((higherY - lowerY) * (value - lowerValue)) / (higherValue - lowerValue)
    );
  };
    ...
    <div className="chart-wrapper">
      ...
      >
        <g transform="translate(15,14)">
          ...
          <Axis
            data={axisCoordinates.D}
            position={AXIS_POSITIONS.D}
            title="D"
          />
          <Axis
            data={axisCoordinates.I}
            position={AXIS_POSITIONS.I}
            title="I"
          />
          <Axis
            data={axisCoordinates.S}
            position={AXIS_POSITIONS.S}
            title="S"
          />
          <Axis
            data={axisCoordinates.C}
            position={AXIS_POSITIONS.C}
            title="C"
          />
      ...
      <p className="axis-title" x="90" y="323.4" textAnchor="middle">
        {title}
      </p>
    </div>
  );
